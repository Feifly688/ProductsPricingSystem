...
def main():
    ...
    # 复制预测结果到 public/results/images 目录（确保目标目录存在）
    os.makedirs(public_save_dir, exist_ok=True)
    ...
    # 读取结果图并转为 base64（与 Java 端约定字段）
    processed_image_b64 = ""
    if processed_image_path and os.path.isfile(processed_image_path):
        with open(processed_image_path, 'rb') as f:
            processed_image_b64 = base64.b64encode(f.read()).decode('ascii')

    # 构造符合 Java 后端期望的字段：detections, detection_time, processed_image
    response_data = {
...
if __name__ == "__main__":
    try:
        ...
        # 任何异常都向 stdout 输出合法 JSON，保证后端能解析且退出码为 0
        fallback = {
            "detections": [],
            "detection_time": 0.0,
            "processed_image": ""
